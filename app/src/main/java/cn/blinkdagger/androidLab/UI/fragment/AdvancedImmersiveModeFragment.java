...
public class AdvancedImmersiveModeFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle state) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            getActivity().getWindow().setStatusBarColor(Color.RED);//状态栏颜色设置为透明。
            ...
            ViewGroup.LayoutParams lp = new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
                    SystemBarUtil.getStatusBarHeight(getActivity()));//获得高和等宽的布局属性
            statusBarView.setBackgroundColor(Color.RED);//设置需要的颜色作为背景填充
    ...
    public void dumpFlagStateToLog(int uiFlags) {
        if ((uiFlags & View.SYSTEM_UI_FLAG_LOW_PROFILE) != 0) {
            Logger.i("SYSTEM_UI_FLAG_LOW_PROFILE is set");
        } else {
            Logger.i("SYSTEM_UI_FLAG_LOW_PROFILE is unset");
        }

        if ((uiFlags & View.SYSTEM_UI_FLAG_FULLSCREEN) != 0) {
            Logger.i( "SYSTEM_UI_FLAG_FULLSCREEN is set");
        } else {
            Logger.i( "SYSTEM_UI_FLAG_FULLSCREEN is unset");
        }

        if ((uiFlags & View.SYSTEM_UI_FLAG_HIDE_NAVIGATION) != 0) {
            Logger.i( "SYSTEM_UI_FLAG_HIDE_NAVIGATION is set");
        } else {
            Logger.i( "SYSTEM_UI_FLAG_HIDE_NAVIGATION is unset");
        }

        if ((uiFlags & View.SYSTEM_UI_FLAG_IMMERSIVE) != 0) {
            Logger.i( "SYSTEM_UI_FLAG_IMMERSIVE is set");
        } else {
            Logger.i( "SYSTEM_UI_FLAG_IMMERSIVE is unset");
        }

        if ((uiFlags & View.SYSTEM_UI_FLAG_IMMERSIVE_STICKY) != 0) {
            Logger.i(TAG, "SYSTEM_UI_FLAG_IMMERSIVE_STICKY is set");
        } else {
            Logger.i(TAG, "SYSTEM_UI_FLAG_IMMERSIVE_STICKY is unset");
        }
