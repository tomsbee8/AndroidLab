...
/**
 * Created by fangjun on 2017/11/30.
 */
object TextTool {
    /**
     * 获取建造者
    ...
    class Builder(private var text: CharSequence) {
        ...
        fun setBackground(drawableId: Int, textColor: Int, textSize: Int): Builder {
            imageSpan = object : ImageSpan(AndroidLabApplication.instance.resources.getDrawable(drawableId)) {
                override fun draw(canvas: Canvas, text: CharSequence, start: Int, end: Int, x: Float, top: Int, y: Int,
                                  bottom: Int, paint: Paint) {
                    paint.typeface = Typeface.create("normal", Typeface.NORMAL)
                    paint.textSize = textSize + 10.toFloat()
                    val len = Math.round(paint.measureText(text, start, end))
                    drawable.setBounds(0, 0, len, textSize + 20)
        ...
        private fun setSpan() {
            ...
            if (imageIsBitmap || imageIsDrawable || imageIsUri || imageIsResourceId) {
                if (imageIsBitmap) {
                    mBuilder.setSpan(ImageSpan(AndroidLabApplication.instance, bitmap), start, end, flag)
                    bitmap = null
                    imageIsBitmap = false
                    ...
                    mBuilder.setSpan(ImageSpan(AndroidLabApplication.instance, uri), start, end, flag)
                    uri = null
                    imageIsUri = false
                } else {
                    mBuilder.setSpan(ImageSpan(AndroidLabApplication.instance, resourceId), start, end, flag)
                    resourceId = 0
