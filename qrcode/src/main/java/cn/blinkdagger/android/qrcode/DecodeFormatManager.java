...
import cn.blinkdagger.android.qrcode.constants.ScanMode;
...
public final class DecodeFormatManager {
  ...
  static {
    ...
    FORMATS_FOR_MODE.put(ScanMode.ONE_D_MODE, ONE_D_FORMATS);
    FORMATS_FOR_MODE.put(ScanMode.PRODUCT_MODE, PRODUCT_FORMATS);
    FORMATS_FOR_MODE.put(ScanMode.QR_CODE_MODE, QR_CODE_FORMATS);
    FORMATS_FOR_MODE.put(ScanMode.DATA_MATRIX_MODE, DATA_MATRIX_FORMATS);
    FORMATS_FOR_MODE.put(ScanMode.AZTEC_MODE, AZTEC_FORMATS);
    FORMATS_FOR_MODE.put(ScanMode.PDF417_MODE, PDF417_FORMATS);
  }
  ...
  public static Set<BarcodeFormat> parseDecodeFormats(Intent intent) {
    ...
    CharSequence scanFormatsString = intent.getStringExtra(ScanMode.FORMATS);
    if (scanFormatsString != null) {
      scanFormats = Arrays.asList(COMMA_PATTERN.split(scanFormatsString));
    }
    return parseDecodeFormats(scanFormats, intent.getStringExtra(ScanMode.MODE));
  }

 public  static Set<BarcodeFormat> parseDecodeFormats(Uri inputUri) {
    List<String> formats = inputUri.getQueryParameters(ScanMode.FORMATS);
    if (formats != null && formats.size() == 1 && formats.get(0) != null) {
      formats = Arrays.asList(COMMA_PATTERN.split(formats.get(0)));
    }
    return parseDecodeFormats(formats, inputUri.getQueryParameter(ScanMode.MODE));
  }
